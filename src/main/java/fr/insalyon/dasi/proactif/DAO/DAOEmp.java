...
public class DAOEmp {
    ...
    public List<Intervention> searchInterventionHistoryEmployer(Employe e) {
        ...
        c.set(SECOND, 59);
        Date end = c.getTime();
        // we search for the interventions from today's date
        Query query = em.createQuery("select i from Intervention i where i.Employe = :emp and i.Horodate >= :sDate and i.Horodate <= :fDate");
